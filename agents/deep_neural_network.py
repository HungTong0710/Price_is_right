...
DOWNLOAD_URL = "https://drive.google.com/drive/folders/1uq5C9edPIZ1973dArZiEO-VE13F7m8MK"
...
class DeepNeuralNetworkInference:
    ...
    def load(self, path):
        if not os.path.exists(path):
            logging.error(f"Model weights file not found at {path}")
            try:
                urllib.request.urlretrieve(DOWNLOAD_URL, path)
                logging.info(f"Model weights file downloaded to {path}")
            except Exception as e:
                logging.error(f"Error downloading model weights file: {e}")
                return
